...
debugLog = function(dat) {};
{
    ...
    debugLog(configLoadMessage + "설정을 불러왔읍니다.");
    checkComplete();
}
...
var loadCss = function(isRetrying) {
    ...
    document.head.appendChild( function() {
        ...
        ret.onload = function() {
                debugLog(configData.themeName + " 테마를 적용했읍니다.");
                checkComplete();
        };
        ret.onerror = function() {
                debugLog("테마 적용에 실패했읍니다.");
                if (isRetrying != true) {
...
if (configData.loadCheerImgs) {
    ...
    debugLog("후원 아이콘을 불러왔읍니다.");
    checkComplete();
}
else {
    debugLog("설정에 의해 후원 아이콘을 불러오지 않았읍니다.");
    checkComplete();
}
...
if (configData.loadTwitchCons) {
    ...
    if (configData.consRealSubsOnly) {
        ...
        debugLog("트위치 이모티콘과 구독콘을 적용했읍니다.");
        checkComplete();
    }
    else {
        ...
        var twitchConsApply = function() {
            ...
            if (isTwitchConsLoaded.subs) {
                ...
                if (isTwitchConsLoaded.emotes) { loadMsg = "트위치 이모티콘" + loadMsg; }
                    ...
                    debugLog("트위치 이모티콘과 구독콘을 적용할 수 없었읍니다.");
                    return;
                }
            }
        ...
        twitchEmoteConRequest.onreadystatechange = function(evt) {
            if (twitchEmoteConRequest.readyState == 4) {
                if (twitchEmoteConRequest.status == 200) {
                    ...
                    debugLog("트위치 이모티콘을 불러왔읍니다.");
                }
                else {
                    debugLog(
                        "트위치 이모티콘을 불러오는 데 실패했읍니다." +
        ...
        twitchSubsConRequest.onreadystatechange = function(evt) {
            if (twitchSubsConRequest.readyState == 4) {
                if (twitchSubsConRequest.status == 200) {
                    ...
                    debugLog("트위치 구독콘을 불러왔읍니다.");
                }
                else {
                    debugLog(
                        "트위치 구독콘을 불러오는 데 실패했읍니다." +
    ...
    debugLog("설정에 따라 트위치 이모티콘과 구독콘을 불러오지 않았읍니다.");
    checkComplete();
}
...
if (configData.loadDcCons) {
    ...
    dcConScript.onload = function() {
        if (dcConsData.length == 0) { debugLog("디씨콘을 불러오는 데 실패했읍니다."); }
        else {
            var keywords = [];
            ...
            debugLog("디씨콘을 적용했읍니다.");
            checkComplete();
        }
    };
}
else {
    debugLog("설정에 따라 디씨콘을 적용하지 않았읍니다.");
    checkComplete();
}
        ...
        lines.forEach( function(element) {
            ...
            // IRC 명령어 처리
                ...
                default:            // 미처리 메세지
                    if (configData.allMessageHandle) {
                        debugLog("처리되지 않은 메세지를 수신했읍니다.<br />" + line);
                    }
                    break;    
                }
            }
            ...
            // 트위치 명령어 처리
                ...
                case "PRIVMSG":            // 채팅 수신
                    ...
                    if ((clip = message.match(/(https?:\/\/)?clips\.[a-zA-Z0-9./]*/g)) != null) {
                ...
                case "NOTICE":            // 공지 메세지
                    ...
                    case "host_target_went_offline":
                                                        // 호스팅이 끊겼을 때
                        debugLog("호스팅이 종료되었읍니다.");
                        break;
                        
                    case "host_on":        // 호스팅되었을 때
                        debugLog(
                            args[5].slice(0,-1) + " 호스팅 중.\n" +
                            "호스팅중인 채팅의 전송은 지원하지 않읍니다.");                    
                        break;
